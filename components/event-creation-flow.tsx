...
import { Store, Calendar, MapPin, Check, Users, Heart } from "lucide-react"
...
export default function EventCreationFlow({ onCancel, commerceId, event }: EventCreationFlowProps) {
          ...
          if (event) {
            ...
            if (associations) {
              const additionalIds = associations
                .map(a => a.commerce_id)
                .filter(id => id !== event.commerce_id)
  ...
  const handleSaveEvent = async () => {
    ...
    try {
      ...
      // Sync junction table (event_commerces)
      ...
      if (allCommerceIds.length > 0) {
        await supabase
          .from('event_commerces')
          .insert(allCommerceIds.map(cid => ({ event_id: eventId, commerce_id: cid })))
      }
  ...
  const SuccessScreen = () => {
    return (
      <div className="space-y-4 text-center py-6">
        ...
        <Button onClick={() => { if (onCancel) onCancel() }} className="mt-4">
                    ...
                    {commerces.length > 1 && form.selectedCommerceId && (
                      <div className="mt-3">
                          ...
                          {commerces
                            ...
                            .map(commerce => (
                              <div key={commerce.id} className="flex items-center space-x-2">
                                ...
                                />
                                <label
                                  htmlFor={`event-commerce-${commerce.id}`}
                        ...
                        <p className="text-xs text-muted-foreground mt-1">
                          {locale === 'fr'
                            ? 'L\'événement apparaîtra aussi sous ces commerces'
                            : 'The event will also appear under these businesses'}
                        </p>
                      </div>
                    )}
                  </>
